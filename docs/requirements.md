...
- 必須項目 `repository`, `branch`, `remote`, `validationCommand`, `releaseTarget` の不足を検出する。
...
参照元 ZIP: `D:\AI\WindowsApp\created_idea_007_git-release-publish-assistant\idea_007_git-release-publish-assistant.zip`。一部に文字化けがあったため、正式docsは `PICKUP_100_IDEAS.md` とドメインガイドから再構成した。
